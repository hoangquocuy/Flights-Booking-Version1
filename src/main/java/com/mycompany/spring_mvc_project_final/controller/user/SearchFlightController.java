...
public class SearchFlightController {
    ...
    public String searchFlightPage(Model model,HttpSession session,
        ...
        try {
            ...
            session.setAttribute("dateDepartTest", flight.getDepartStr());
            session.setAttribute("direction", "trip");
            session.setAttribute("departure", departure);
            session.setAttribute("destination", destination);
            session.setAttribute("departureId", flight.getFlightRoute().getDeparture().getId());
            session.setAttribute("destinationId", flight.getFlightRoute().getDestination().getId());
    ...
    @RequestMapping(value = {"/search-flight-next"}, method = RequestMethod.GET)
    public String searchFlightArrivalPage(Model model,HttpSession session,
            @SessionAttribute("dateDepartTest") String dateDepartTest,
            @SessionAttribute("departureId") int departureId,
            @SessionAttribute("destinationId") int destinationId,
            @RequestParam("departNextDate") String departNextDate
    ) throws ParseException {
        Date dt = new SimpleDateFormat("dd/MM/yyyy").parse(departNextDate);

        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
        String dateFormat = formatter.format(dt);
        List< FlightsEntity> flights = flightService.getFlightsDipart(dateFormat, departureId, destinationId);
        flights = flightService.flights(flights, bookingDetailService);
        Collections.sort(flights, new SearchService());
        session.setAttribute("flights", flights);
        return "user/ajax/load-next-search";
    
    }
    
    @RequestMapping(value = {"/search-flight-arrival"}, method = RequestMethod.GET)
    public String searchFlightNextPage(Model model,HttpSession session,
